...
```{r remove-redundant}
df <- df %>% select(-UDI, -`Product ID`, -`Air temperature [K]`, -`Process temperature [K]`, -`Failure Type`)
...
### 5.5.1 Interpretación de los coeficientes (β) del modelo

Una de las principales ventajas de la regresión logística es su interpretabilidad. Al tratarse de un modelo lineal, cada predictor tiene un coeficiente asociado (β), que representa su contribución al logaritmo de las probabilidades (logit).

```{r model-coefficients}
summary(model_log$finalModel)
```
### 5.3.2 Regularización con Lasso (L1) para selección de variables

Para comprobar si el modelo puede optimizarse o eliminar variables redundantes, se ha aplicado regularización tipo Lasso (L1). Esta técnica penaliza los coeficientes más pequeños y tiende a dejarlos en cero, lo que permite realizar selección automática de variables.

```{r lasso-regularization}
library(glmnet)

# Preparar los datos como matriz para glmnet
...
# Visualizar error vs lambda
plot(cv_lasso)

# Extraer coeficientes con lambda óptimo
...
Este proyecto demuestra que, con un preprocesado cuidadoso y un modelo interpretable
(regresión logística), es posible **predecir con exactitud los fallos de motores
eléctricos** en un entorno industrial simulado.

1. **Separabilidad del problema.**  
   Las variables operativas (torque, velocidad y desgaste) contienen señales
   suficientemente fuertes como para obtener AUC = 1 incluso con solo un 25 %
   de los datos de entrenamiento. La frontera lineal es estable y la multicolinealidad
   es despreciable (VIF ≈ 1).

2. **Balanceo de clases.**  
   Se evaluaron upsampling, SMOTE y undersampling. Todas logran métricas
   perfectas, pero el upsampling aleatorio se adopta por ser el método más sencillo,
   auditable y sin generación de datos sintéticos.

3. **Evaluación exhaustiva.**  
   La combinación de matriz de confusión, curva ROC, curva de calibración y
   validación cruzada 10-fold confirma la **ausencia de sobreajuste** dentro del
   dominio simulado. Las probabilidades están perfectamente calibradas, aunque son
   extremas (0 o 1), lo que sugiere usar umbrales adaptativos o recalibración con
   datos reales si se necesita graduar riesgos.

4. **Prueba de robustez con muestra reducida.**  
   Diez réplicas con solo el 25 % del *train* mantuvieron AUC = 1 (sd = 0),
   indicando que el tamaño de muestra no es un factor crítico mientras la
   distribución de variables se conserve.

5. **Comparativa de algoritmos.**  
   Naïve Bayes con kernel se aproxima al rendimiento de la regresión logística
   (99.9 % de accuracy) demostrando que el preprocesado es más determinante que
   la elección del clasificador en este dataset.

---

### Líneas de trabajo futuras

* **Validación externa** con datos reales, no balanceados, para verificar que la
  separación perfecta no es artefacto de un entorno simulado.
* **Predicción multiclase** del tipo de fallo (`Failure Type`) empleando modelos
  como Random Forest o XGBoost y evaluando métricas *per-class*.
* **Re-calibración** mediante *Platt scaling* o regresión isotónica si en planta
  se requieren probabilidades graduadas (por ejemplo, para planificar mantenimientos
  en función de un nivel de riesgo).
* **Análisis temporal**: incorporar series de sensores para anticipar fallos con
  mayor antelación y detectar deriva del proceso.

En resumen, el trabajo cumple los objetivos de la Unidad 25: aplica los principios
del aprendizaje automático para diseñar una **máquina inteligente** capaz de
diagnosticar su propio estado y planificar actuaciones de mantenimiento. La
metodología es transparente, las decisiones están justificadas y se dejan claras
las limitaciones y los pasos necesarios para llevar el modelo a un entorno de
producción real.
