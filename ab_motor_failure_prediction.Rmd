...
El conjunto de datos utilizado en este trabajo proviene de un entorno industrial simulado para el estudio del mantenimiento predictivo de motores eléctricos. Fue obtenido originalmente de la plataforma Kaggle y contiene un total de **1.000 registros** y **10 variables** que representan mediciones relevantes para diagnosticar el estado de funcionamiento de un sistema rotativo.
...
Estas gráficas permiten visualizar posibles relaciones lineales o no lineales, detectar patrones de agrupación y estimar la fuerza de correlación entre pares de variables. 

- El mapa de calor muestra una **fuerte correlación positiva entre la temperatura del aire y del proceso**, lo que es coherente en un sistema térmico. También hay una correlación moderada entre `Torque` y `Tool wear`, lo cual podría indicar una asociación entre carga aplicada y desgaste.

- El gráfico `ggpairs` es útil para identificar relaciones lineales o agrupamientos, como la posible asociación entre temperaturas elevadas y valores bajos de torque. Estas combinaciones podrían estar relacionadas con condiciones de fallo o baja eficiencia, lo que justifica su inclusión como variables predictoras. Este análisis también ayuda a identificar variables con comportamientos redundantes o que aportan poca información, de cara a su posible eliminación en la fase de preparación de datos.
...
- Aggarwal, H. (2020). *Predictive Maintenance Dataset*. Kaggle. Disponible en: https://www.kaggle.com/datasets/hiimanshuagarwal/predictive-maintenance-dataset
